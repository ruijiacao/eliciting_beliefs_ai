...
def format_amazon(text, label):
    return "A customer wrote the following review:\n{}\nThe sentiment in this review is {}.".format(
        text, ["negative", "positive"][label]
    )
...
neg_hs, pos_hs, y, all_text = make_training_data(
    model, tokenizer, data, format=format_amazon, n=200
)
...
def classify_single(probe, hs):
    ...
    classifier_direction = np.squeeze(
        np.transpose(probe[0].weight.detach().cpu().numpy())
    )
...
neg_credences = np.apply_along_axis(
    lambda x: np.dot(x, classifier_direction), 1, neg_hs_test
)
pos_credences = np.apply_along_axis(
    lambda x: np.dot(x, classifier_direction), 1, pos_hs_test
)
...
plt.hist(
    [
        pos_credences[[(not not x) for x in y_test]],
        pos_credences[[not x for x in y_test]],
    ],
    stacked=True,
    density=True,
)
...
plt.hist(
    [
        neg_credences[[(not not x) for x in y_test]],
        neg_credences[[not x for x in y_test]],
    ],
    stacked=True,
    density=True,
)
...
for i in range(5):
    probe, loss = ccs(
        residual_neg_hs_train, residual_pos_hs_train, ntries=5, loss_func=ccs_loss
    )

    ccs_acc = get_acc(probe, residual_neg_hs_test, residual_pos_hs_test, y_test)
    ccs_train_acc = get_acc(
        probe, residual_neg_hs_train, residual_pos_hs_train, y_train
    )

    print(
        "CCS accuracy (component {}): {} in training, {} in testing, {} loss".format(
            i, ccs_train_acc, ccs_acc, loss
        )
    )
    ...
    residual_neg_hs_train = np.apply_along_axis(
        lambda x: perp(x, classifier_direction), 1, residual_neg_hs_train
    )
    residual_pos_hs_train = np.apply_along_axis(
        lambda x: perp(x, classifier_direction), 1, residual_pos_hs_train
    )
    residual_neg_hs_test = np.apply_along_axis(
        lambda x: perp(x, classifier_direction), 1, residual_neg_hs_test
    )
    residual_pos_hs_ttest = np.apply_along_axis(
        lambda x: perp(x, classifier_direction), 1, residual_pos_hs_test
    )
...
hsp = llama_hs_from_text(
    model,
    tokenizer,
    "The following statement is true: " + tokenizer.decode(outputs.sequences[0]),
)
hsn = llama_hs_from_text(
    model,
    tokenizer,
    "The following statement is false: " + tokenizer.decode(outputs.sequences[0]),
)
print(hsp)
...
for tok, score in zip(generated_tokens[0], transition_scores[0]):
    ...
    print(
        f"{tok:5d} | {tokenizer.decode(tok):8s} | {score.cpu().numpy():.3f} | {np.exp(score.cpu().numpy()):.2%}"
    )
...
def model_directed(text, merge, verbose=True):
    ...
    if verbose:
        ...
        for i in range(k):
            print(
                "{}: {}(prob={:.5f}, truth={:.5f}) = {} ".format(
                    tokens[resorted[i]],
                    score.__name__,
                    probs[resorted[i]],
                    truthiness[resorted[i]],
                    scores[resorted[i]],
                )
            )
...
for score in [
    ...
    product,
    truth,
]:  # [prob, product, truthHeavy, truthHeavier, truth, false, falser, falsest]:
    print("")
    text = prompt
    for i in range(20):
